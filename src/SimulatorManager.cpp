...
namespace rtt::robothub::simulation {
    ...
    void SimulatorManager::callRobotControlFeedbackCallback(RobotControlFeedback& feedback) {
        ...
        if (this->robotControlFeedbackCallback != nullptr)
            this->robotControlFeedbackCallback(feedback);
    }

    void SimulatorManager::callConfigurationFeedbackCallback(ConfigurationFeedback& feedback) {
        // Only call if the callback function has been set
        if (this->configurationFeedbackCallback != nullptr)
            this->configurationFeedbackCallback(feedback);
    }
    ...
    RobotControlFeedback SimulatorManager::getControlFeedbackFromDatagram(QNetworkDatagram& datagram, bool isTeamYellow) {
        ...
        proto::RobotControlResponse response;
        ...
        for (const proto::RobotFeedback &robotFeedback : response.feedback()) {
            // This boolean is optional, so first check if it has been set before checking if it is set to true
            if (robotFeedback.has_dribbler_ball_contact() && robotFeedback.dribbler_ball_contact())
                feedback.robotsThatHaveTheBall.push_back(robotFeedback.id());
        }

        for (const proto::SimulatorError &simulatorError : response.errors()) {
            SimulationError error;
            if (simulatorError.has_code())
                error.code = simulatorError.code();
            if (simulatorError.has_message())
                error.message = simulatorError.message();
    ...
    ConfigurationFeedback SimulatorManager::getConfigurationFeedbackFromDatagram(QNetworkDatagram& datagram) {
        // First, parse datagram into the proto packet that it resembles
        proto::SimulatorResponse response;
        response.ParseFromArray(datagram.data().data(), datagram.data().size());

        // Then, put the information in an easy accesible struct
        ConfigurationFeedback feedback;
        for (const proto::SimulatorError &simulationError : response.errors()) {
            SimulationError error;
            if (simulationError.has_code())
                error.code = simulationError.code();
            if (simulationError.has_message())
                error.message = simulationError.message();
            feedback.simulationErrors.push_back(error);
        }
        return feedback;
    }
    ...
    void SimulatorManager::stopFeedbackListeningThreads() {
        ...
        if (this->blueFeedbackListenThread.joinable())
            this->blueFeedbackListenThread.join();
        
        if (this->yellowFeedbackListenThread.joinable())
            this->yellowFeedbackListenThread.join();
        
        if (this->configurationFeedbackListenThread.joinable())
            this->configurationFeedbackListenThread.join();
